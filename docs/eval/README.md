...
- `confirm_set.json` — fresh problems (sha1 of the problem text does not appear
  in any saved `runs/` row). **Read once per promotion decision** — a config
  that wins on the dev set or quick screen gets one run here before it can
  become the default. Never tune on it.
