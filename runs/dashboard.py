...
Also: `python runs/dashboard.py --export docs/charts` writes the four charts as
standalone SVGs and the live-experiments table as docs/charts/experiments.md."""
...
def stats(d):
    ...
    miss_judge = sum(
        not r["metacog"]["correct"] and any(c["correct"] for c in r["metacog"]["candidates"])
        for r in d
    )
    casc = [r for r, g in zip(d, gens, strict=True) if g == 1]
    return dict(
        n=n,
        b=b,
        mc=mc,
...
def fmt(s, total=None):
    ...
    return (
        f"<td>{prog}</td><td>{s['b']}/{n} ({s['b'] / n:.0%})</td><td><b>{s['mc']}/{n} ({s['mc'] / n:.0%})</b></td>"
...
HEAD = (
    "<tr><th>thinker</th><th>problems</th><th>baseline</th><th>MetaCog</th><th>ceiling<br>(any path)</th>"
...
def bar_svg(title, groups, series, colors, ymax, fmt_v, w=560, h=230):
    """Grouped bar chart as a standalone SVG. groups: labels; series: {name: [v per group]}."""
    left, top, bot = 40, 30, 50
    gw = (w - left - 10) / len(groups)
    bw = gw / (len(series) + 1)
    svg = [
        f'<svg xmlns="http://www.w3.org/2000/svg" width={w} height={h} font-family=system-ui font-size=11>',
        f"<text x={left} y=14 font-weight=bold font-size=13>{title}</text>",
    ]
    for f in (0, 0.25, 0.5, 0.75, 1):
        y = top + (h - top - bot) * (1 - f)
        svg.append(
            f"<line x1={left} x2={w - 10} y1={y:.0f} y2={y:.0f} stroke=#eee />"
    ...
    for gi, g in enumerate(groups):
        for si, vals in enumerate(series.values()):
            ...
            y = h - bot - bh
            svg.append(
                f"<rect x={x:.0f} y={y:.0f} width={bw - 2:.0f} height={bh:.0f} fill={colors[si]} />"
                f"<text x={x + bw / 2 - 1:.0f} y={y - 3:.0f} text-anchor=middle>{fmt_v(v)}</text>"
            )
        svg.append(
            f"<text x={left + (gi + 0.5) * gw:.0f} y={h - bot + 14} text-anchor=middle>{g}</text>"
        )
    lx = left
    for si, name in enumerate(series):
        svg.append(
            f"<rect x={lx} y={h - 14} width=10 height=10 fill={colors[si]} /><text x={lx + 14} y={h - 5}>{name}</text>"
        )
        lx += 14 + 7 * len(name) + 16
    svg.append("</svg>")
    # quote attribute values so the output is also well-formed standalone SVG/XML
    return re.sub(r"=([^\"'\s>][^\s>]*)", r'="\1"', "".join(svg))


def bars(title, note, groups, series, colors, ymax, fmt_v, w=560, h=230):
    """Chart wrapped for the dashboard: SVG plus its caption."""
    svg = bar_svg(title, groups, series, colors, ymax, fmt_v, w, h)
    return f"<div class=chart>{svg}<small>{note}</small></div>"


def chart_parts(per_bench):
    groups = [
        t.split(" (")[0].replace("Custom harder counting set", "harder") for t, _ in per_bench
    ]
    ...
    return [
        (
            "accuracy",
            "1. Accuracy: baseline → MetaCog → ceiling",
            ...
            groups,
            {
                "baseline": [s["b"] / s["n"] for s in S],
                "MetaCog": [s["mc"] / s["n"] for s in S],
        ...
        (
            "misses",
            "2. Why MetaCog still misses (share of problems)",
            ...
            groups,
            {
                "no path right": [s["miss_cov"] / s["n"] for s in S],
                "Jev picked wrong": [s["miss_judge"] / s["n"] for s in S],
            },
            ["#e88", "#fc6"],
            max(0.05, max(s["miss_cov"] / s["n"] for s in S) * 1.2),
            pct,
        ),
        (
            "cost",
            "3. Cost vs one greedy answer (× multiplier)",
            ...
            groups,
            {
                "thinker gens": [s["gens"] / s["n"] for s in S],
                "wall time": [
                    (s["sm"] / s["nsm"]) / (s["sb"] / s["nsb"]) if s["nsb"] and s["nsm"] else 0
                    for s in S
                ],
            },
            ["#69c", "#c96"],
        ...
        (
            "cascade",
            "4. Cascade: problems stopped after the greedy path",
            ...
            groups,
            {
                "stopped early": [s["casc"] / s["n"] for s in S],
...
def charts(per_bench):
    body = "".join(
        bars(title, note, groups, series, colors, ymax, fmt_v)
        for _, title, note, groups, series, colors, ymax, fmt_v in chart_parts(per_bench)
    )
    return f"<div class=charts>{body}</div>"
...
EXPERIMENTS = {
    "prior": (
        "B",
        "Answer prior",
        "Current MetaCog, plus: Jev also rates each distinct final answer on its own (no reasoning shown) and that is added to the score before picking.",
    ),
    "adaptP": (
        "B",
        "Answer prior + adaptive paths",
        "Answer-prior pick on top of adaptive branching (2–6 full answers when Jev is unsure).",
    ),
    "adaptN": (
        "A",
        "Adaptive paths",
        "Answer once; if Jev is ≥95% sure, stop. Otherwise write 2–6 more full answers (more when Jev is less sure) and let Jev pick.",
    ),
    "sketch": (
        "A",
        "Adaptive sketches",
        "Same, but the extra attempts are 800-token outlines; Jev keeps the best 1–3 and only those are written out in full.",
    ),
    "sketchC": ("A", "Cheaper sketches", "500-token outlines, at most 2 written out in full."),
    "adaptW": (
        "A",
        "Wider & earlier",
        "Stop at 90% sure instead of 95%; branch up to 8 full answers when unsure.",
    ),
    "deep": (
        "A",
        "Deep tree",
        "4–8 outlines of 500 tokens; Jev prunes; best 1–2 written in full; if Jev is still <95% sure, branch again from the best answer — up to 3 levels.",
    ),
    "divP": (
        "A",
        "Diverse thought paths",
        "Answer prior + adaptive paths, but each extra thought path is prompted with a different approach (eliminate options / verify a second way / work backwards…) so the paths don't all make the same mistake.",
    ),
    "escP": (
        "A",
        "Escalate to a bigger model",
        "Answer prior + adaptive paths; if Jev is still <95% sure after branching, one more thought path is written by a stronger model (DeepSeek → Kimi, Kimi → DeepSeek) and joins the pool.",
    ),
    "triage": (
        "C",
        "Triage (full stack)",
        "Jev first rates how hard the problem is from the statement alone; hard problems start 2–6 answers at the same time as the first one (no waiting), easy ones stay single-answer. Uses adaptive + answer prior.",
    ),
}
GROUPS = {
    "A": (
        "1 · How many answers to write, and when",
        "Changes the generation side: spend more attempts only where Jev is unsure. Goal: accuracy at equal or lower cost.",
    ),
    "B": (
        "2 · How to pick the winner",
        "Changes the judging side only: same attempts, better selection. Goal: accuracy at ~zero extra cost.",
    ),
    "C": (
        "3 · How to be faster",
        "Changes scheduling: decide effort up front and run attempts in parallel. Goal: wall-clock time.",
    ),
}
...
def pval(f, l_):
    ...
    if not n:
        return 1.0
    return min(1.0, 2 * sum(comb(n, i) for i in range(min(f, l_) + 1)) / 2**n)


def experiment_rows():
    """Paired stats per experiment config; one dict per config with rows."""
    out = []
    for key, (group, name, desc) in EXPERIMENTS.items():
        ...
        fixed = sum(
            e["metacog"]["correct"] and not r["metacog"]["correct"]
            for e, r in zip(exp, ref, strict=True)
        )
        lost = sum(
            r["metacog"]["correct"] and not e["metacog"]["correct"]
            for e, r in zip(exp, ref, strict=True)
        )
...
def experiments():
    ...
    for r in experiment_rows():
        name, desc, n = r["name"], r["desc"], r["n"]
        acc_b, acc_r, acc_e = r["acc_b"], r["acc_r"], r["acc_e"]
        fixed, lost, fb, lb, p = r["fixed"], r["lost"], r["fb"], r["lb"], r["p"]
        sec_e, sec_r, tok_e, tok_r = r["sec_e"], r["sec_r"], r["tok_e"], r["tok_r"]
        verdict = (
            "<b style=color:#1a7f1a>better</b>"
            if p < 0.05 and fixed > lost
            else "<b style=color:#b00>worse</b>"
            if p < 0.05
            else f"leaning {'better' if fixed > lost else 'worse' if lost > fixed else 'equal'}, not yet significant"
        )
        cls = "up" if fixed > lost else "down" if lost > fixed else ""
        rows[r["group"]].append(
            (
                p if fixed > lost else 2 - p,
    ...
    return (
        "<h2>Live experiments — new configurations vs current MetaCog</h2>"
...
def per_bench_stats():
    """[(title, aggregate stats)] per benchmark, pooled over MODELS."""
    per_bench = []
    for key, (title, _total) in BENCHES.items():
        agg = {}
        for m in MODELS:
            s = stats(load(key, m))
            if s["n"]:
...
def experiments_md():
    """The live-experiments table as markdown (for docs/README)."""
    lines = [
        "| configuration | problems | model alone | current MetaCog | new config | fixed/lost vs current | time | tokens | p |",
        "|---|---|---|---|---|---|---|---|---|",
    ]
    for r in experiment_rows():
        lines.append(
            f"| {r['name']} | {r['n']} | {100 * r['acc_b'] / r['n']:.1f}% | "
            f"{100 * r['acc_r'] / r['n']:.1f}% | {100 * r['acc_e'] / r['n']:.1f}% | "
            f"+{r['fixed']} / −{r['lost']} | {r['sec_e'] / r['sec_r']:.2f}× | "
            f"{r['tok_e'] / r['tok_r']:.2f}× | {r['p']:.2f} |"
        )
    return "\n".join(lines) + "\n"


def export(outdir):
    """Write the four charts as standalone SVGs + experiments.md to `outdir`."""
    os.makedirs(outdir, exist_ok=True)
    for name, title, _note, groups, series, colors, ymax, fmt_v in chart_parts(per_bench_stats()):
        svg = bar_svg(title, groups, series, colors, ymax, fmt_v)
        open(os.path.join(outdir, f"{name}.svg"), "w").write(svg)
        print(f"wrote {outdir}/{name}.svg")
    md = experiments_md()
    open(os.path.join(outdir, "experiments.md"), "w").write(md)
    print(md)


def build():
    out = [
        f"<!doctype html><meta charset=utf-8><meta http-equiv=refresh content=60><title>MetaCog benchmark results</title><style>{CSS}</style>",
    ...
    for key, (title, total) in BENCHES.items():
        ...
        sections.append(
            f"<h2>{title}</h2><table>{HEAD}{''.join(rows)}<tr class=sum><th>all</th>{fmt(agg).replace('<td>', '<th>').replace('</td>', '</th>')}</tr></table>"
        )
    out.append(experiments())
    out.append(charts(per_bench))
    out.append(
        f"<h2>Summary (pooled over thinkers)</h2><table>{HEAD.replace('thinker</th>', 'benchmark</th>')}{''.join(summary)}"
...
if __name__ == "__main__":
    if "--export" in sys.argv:
        export(
            sys.argv[sys.argv.index("--export") + 1]
