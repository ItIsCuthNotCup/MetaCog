...
PREAMBLE = "We need to solve the problem of finding the average speed of a train journey.\n\n"
...
def test_multi_approach_splits_into_self_contained_paths():
    ...
    assert len(parts) == 2
    for part in parts:
        assert part.startswith(PREAMBLE.strip().splitlines()[0][:10])
        assert "average speed" in part
...
def test_single_marker_returns_original():
    text = PREAMBLE + PIECE_A
    assert split_paths(text) == [text]
...
def test_mid_sentence_marker_after_period():
    ...
    text = (
        ...
        + long_a
        + (
            "Alternatively, we could instead try another route entirely, "
    ...
    parts = split_paths(text)
    assert len(parts) == 2
    assert parts[0].endswith("length.")
    assert "Approach 3:" in parts[1]
